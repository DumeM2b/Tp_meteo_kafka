...
from pyspark.sql.functions import (
    col, from_json, from_unixtime, to_timestamp, expr, to_json, struct, lit, hour, when
)
...
# Calcul de l'indice de chaleur (heat_index)
...
# Calcul de l'indice de gravité (severity_index)
processed_df = processed_df.withColumn(
    "severity_index",
    (col("speed") * 0.5) + ((1015 - col("pressure")) * 0.3) + (col("humidity") * 0.2)
)

# Catégorisation de la période de la journée (time_of_day)
processed_df = processed_df.withColumn(
    "time_of_day",
